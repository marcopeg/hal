...
- **Config directory:** by default the wizard uses the current working directory. Use `--config <path>` to run the wizard (and write config) in another directory.
...
- **Secrets:** the wizard can store the bot token (and user IDs) in env files and put placeholders in the config so secrets aren’t committed. The docs use `.env` as the standard file and you should keep it out of git. The wizard writes to a single env file in the config directory (default: the current working directory, or `--config` when set). When an existing `.env` or `.env.local` is found, the wizard prompts you to choose which file to use (or skip env writing). Bot token placeholders are per-project (based on the project key), e.g. `${MYBOT_TELEGRAM_TOKEN}`; user IDs use `${TELEGRAM_USER_ID}`, `${TELEGRAM_USER_ID_2}`, … See [Env files — Wizard file selection](../config/env-files/README.md#wizard-file-selection) for the full selection rules.
