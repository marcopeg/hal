...
- **Secrets:** the wizard can write the bot token (and user IDs) into the config directory’s `.env` and put `${TELEGRAM_BOT_TOKEN}` (and similar) in the config so secrets aren’t committed.
