...
If the user provides only the task (no other text or context), do not create or modify any files. Instead, respond asking them to provide the full context for the task, or to reply with “cancel”, “abort”, or another general negative response to cancel the task.
